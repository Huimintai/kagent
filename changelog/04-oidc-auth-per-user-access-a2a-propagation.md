# feat: add OIDC auth, per-user agent access control, and A2A auth propagation
...
### 4. Controller Watch Improvements
Added `AnnotationChangedPredicate` so the agent controller reconciles when annotations
(user-id, private-mode) change — not just on spec changes. Without this, annotation-only
updates would be silently ignored by the reconciler.
...
| **API Types** | `go/api/adk/types.go` — SAPAICore model type, SessionTokenLabel, AuthUrl |
| **Database** | `go/api/database/models.go` — `UserID` and `PrivateMode` fields on Agent |
| **HTTP Types** | `go/api/httpapi/types.go` — SAPAICoreParams, UserID/PrivateMode in responses |
| **Controller** | `agent_controller.go` — AnnotationChangedPredicate; `reconciler.go` — reads access annotations |
| **Auth Middleware** | `authn.go` — X-MCP-Token-* collection, X-User-Id forwarding |
...
| **Claude Skills** | New `kagent-env-kind` skill for SAPAICore credential setup |
