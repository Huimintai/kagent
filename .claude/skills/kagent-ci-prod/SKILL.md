...
```bash
# Change feature flags without rebuild (patch ConfigMap directly, avoid helm upgrade)
kubectl patch configmap kagent-ui-config -n kagent --type merge -p '{
  "data": {
    "KAGENT_ALLOWED_NAMESPACE": "dbci-agent",
    "KAGENT_DISABLE_MODEL_CREATION": "true",
    "KAGENT_DISABLE_MCP_SERVER_CREATION" : "true",
    "KAGENT_DISABLE_BYO_AGENT_CREATION" : "true",
    "KAGENT_PROTECTED_AGENTS": ""
  }
}'
kubectl rollout restart deployment/kagent-ui -n kagent
## Rollback
