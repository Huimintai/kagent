...
func TestBuildBootstrapJSON_HTTPBaseURL_InjectsNoProxy(t *testing.T) {
	...
	require.Equal(t, wantNoProxy, env["NO_PROXY"], "NO_PROXY must include cluster host")
	// HTTP_PROXY and ALL_PROXY (both cases) must be cleared so Node.js / undici EnvHttpProxyAgent
	// does not tunnel HTTP model requests through the sandbox HTTPS proxy.
