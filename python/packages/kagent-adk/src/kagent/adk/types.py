...
def create_header_provider(
    ...
    session_token_label: str | None = None,
    ...
    Args:
        ...
        session_token_label: Optional MCP server label for per-user token injection.
                            When set, reads a token from session state under the key
                            ``mcp_token:<session_token_label>`` and injects it as
                            ``Authorization: Bearer <token>``.  This takes the lowest
                            priority — STS tokens always override it.  Intended for
                            use with the ``set_mcp_token`` built-in tool.

    Returns:
        A header provider function, or None if no headers need to be propagated.
    """
    if not allowed_headers and not sts_header_provider and not session_token_label:
        return None
    ...
    # Pre-compute the session state key for the per-user token (avoids string fmt on every call)
    from kagent.adk.tools.set_mcp_token_tool import mcp_token_state_key

    session_token_key = mcp_token_state_key(session_token_label) if session_token_label else None

    def header_provider(readonly_context: Optional[ReadonlyContext]) -> dict[str, str]:
        headers: dict[str, str] = {}

        # 1. Per-user session token (lowest priority)
        if session_token_key and readonly_context:
            user_token = readonly_context.state.get(session_token_key)
            if user_token:
                headers["Authorization"] = f"Bearer {user_token}"

        # 2. Allowed headers from A2A request (override session token for same header names)
        if normalized_allowed and readonly_context:
            ...
            for header_name, header_value in request_headers.items():
                if header_name.lower() in normalized_allowed:
                    ...
                    keys_to_remove = [k for k in headers if k.lower() == header_name.lower()]
                    for k in keys_to_remove:
                        del headers[k]
                    headers[header_name] = header_value

        # 3. STS headers (highest priority — always override everything else)
...
class HttpMcpServerConfig(BaseModel):
    ...
    session_token_label: str | None = None  # MCP server label for per-user token (set via set_mcp_token tool)
        ...
        if self.http_tools:
            for http_tool in self.http_tools:  # add http tools
                ...
                tool_header_provider = create_header_provider(
                    ...
                    session_token_label=http_tool.session_token_label,
                )
                tools.append(
                    KAgentMcpToolset(
        ...
        if self.sse_tools:
            for sse_tool in self.sse_tools:  # add sse tools
                ...
                tool_header_provider = create_header_provider(
                    ...
                    sts_header_provider=sts_header_provider,
                    session_token_label=sse_tool.session_token_label,
                )
                tools.append(
                    KAgentMcpToolset(
