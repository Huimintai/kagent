...
- Add comment system on agent cards for peer feedback and collaboration
- `POST/GET/DELETE /api/agents/{ns}/{name}/comments` — full CRUD API with ownership-based delete
- Comments display user Identicon avatars, relative timestamps, and optimistic UI updates
- 500 character limit with character counter, Cmd+Enter shortcut to post
- Database migration: `000007_agent_comments` (agent_comment table with indexes)
- AgentComments component integrated into AgentDetailsSidebar
- MessageSquare icon on AgentCard as comment count indicator

### Dashboard — Agent Leaderboard & Hot MCPs
- Add `/dashboard` page with gamified leaderboard to encourage platform engagement
- Top agents ranked by session and message count with medal icons (gold/silver/bronze)
- Hot MCPs ranked by tool count with fire indicators for top 3
- Platform summary stat cards (total agents, sessions, MCPs, sessions today)
- `GET /api/stats?limit=N` endpoint (max 50) for leaderboard data
- Feature-flagged via `KAGENT_DISABLE_DASHBOARD` (enabled by default)
- Responsive design with proportional activity bars
