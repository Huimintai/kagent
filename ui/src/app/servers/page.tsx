...
import { ToolServerResponse, ToolServerCreateRequest, RemoteMCPServer, MCPServer } from "@/types";
...
export default function ServersPage() {
  ...
  const handleAddServer = async (data: { name: string; namespace: string; url: string; type: string }) => {
    const serverRequest: ToolServerCreateRequest = {
      type: data.type as ToolServerCreateRequest["type"],
      remoteMCPServer: data.type === "RemoteMCPServer" ? { metadata: { name: data.name, namespace: data.namespace }, spec: { url: data.url } } as unknown as RemoteMCPServer : undefined,
      mcpServer: data.type === "MCPServer" ? { metadata: { name: data.name, namespace: data.namespace }, spec: { url: data.url } } as unknown as MCPServer : undefined,
    };
    try {
      setIsLoading(true);
