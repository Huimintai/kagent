...
export async function getCurrentUserId() {
  ...
  if (typeof window === "undefined") {
    ...
    // Server-side fallback: read from cookie set by the client-side userStore.
    try {
      const { cookies } = await import("next/headers");
      const cookieStore = await cookies();
      const cookieUserId = cookieStore.get(USER_ID_KEY)?.value;
      if (cookieUserId && cookieUserId !== DEFAULT_USER_ID) {
        return decodeURIComponent(cookieUserId);
      }
    } catch {
      // Ignore
    }
