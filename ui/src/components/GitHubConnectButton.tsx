  ...
  useEffect(() => {
    fetch("/actions/api/auth/github/status")
      ...
      .then((data) => {
        ...
        // Filter out disabled instances — they should not appear in the UI at all
        const fetched: InstanceStatus[] = (data.instances as InstanceStatus[]).filter((i) => !i.disabled);
        ...
        <>
          ...
          >
            ...
            <span className="inline-grid [&>span]:col-start-1 [&>span]:row-start-1">
              <span className="group-hover:invisible">{inst.label}</span>
              <span className="invisible group-hover:visible">Disconnect</span>
            </span>
          </Button>
